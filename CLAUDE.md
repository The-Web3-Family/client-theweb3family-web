...
- External dependencies are Google Fonts, the booking embed, and Microsoft Clarity. That is the whole list of things a visitor's browser talks to. `@11ty/eleventy` is a dev-time dependency only; it never ships to the browser. ZipForm and GoatCounter will each become a dependency once their real credentials are wired in (see section 8); neither is live yet.
